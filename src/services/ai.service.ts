...
const mockTranscribeAudio = async (noteId: string): Promise<string> => {
...
const mockGenerateSummary = async (transcribedText: string): Promise<string> => {
  // 1. Define the System Instruction (AI Persona/Task)
  const systemPrompt =
    'You are a specialized medical summarization AI. Condense the following patient consultation transcript into a structured, concise medical summary suitable for charting. Focus on Chief Complaint, History, Findings, and Action Plan. Use abbreviations (e.g., y/o, w/) where appropriate.';
...
export const generateAISummary = async (noteId: string): Promise<string> => {
